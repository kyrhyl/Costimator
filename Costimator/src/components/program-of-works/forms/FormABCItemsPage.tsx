  ...
  return (
    <A4PageWrapper pageNumber="ABC-2">
      ...
      <table className="itemized-table text-[8px] w-full">
        ...
        <tbody>
          {data.parts.flatMap((part, partIndex) => {
            ...
            part.items.forEach((item, index) => {
              rows.push(
                <tr key={`abc-items-line-eval-${part.part}-${item.payItemNumber}-${index}`}>
              ...
              rows.push(
                <tr key={`abc-items-line-sub-${part.part}-${item.payItemNumber}-${index}`}>
